...
@method_decorator(csrf_exempt, name='dispatch')
class MyFridgeImageUploadView(APIView):
    def post(self, request, format=None):
        serializer = MyFridgeFileSerializer(data=request.data)
        if serializer.is_valid():
            file_path = serializer.save()
            file_url = request.build_absolute_uri(settings.MEDIA_URL + file_path)
            return Response({'file_url': file_url}, status=status.HTTP_201_CREATED)
    ...
    def get(self, request, file_name):
        file_path = os.path.join(settings.MYFRIDGE_MEDIA_ROOT, file_name)
        if os.path.exists(file_path):
            file = open(file_path, 'rb')
            response = FileResponse(file)
            return response
...
@method_decorator(csrf_exempt, name='dispatch')
class DowellCubesImageUploadView(APIView):
    def post(self, request, format=None):
        serializer = DowellCubesFileSerializer(data=request.data)
        if serializer.is_valid():
            file_path = serializer.save()
            file_url = request.build_absolute_uri(settings.MEDIA_URL + file_path)
            return Response({'file_url': file_url}, status=status.HTTP_201_CREATED)
